...
export default function HistoryScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      Promise.all([loadSessions(), loadWorkoutPlan(), loadCardioConfig()]).then(
        ([s, savedPlan, savedCardioConfig]) => {
          setSessions(s);
  ...
  const deleteSingleSession = (key) => {
    Alert.alert("Delete session", `Delete session from ${formatDate(key)}?`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete",
        style: "destructive",
    ...
    const existing = Object.keys(session).filter(
      (k) =>
        ...
        (Array.isArray(session[k]) ||
          (typeof session[k] === "object" && session[k] !== null)),
    );
    const unique = [];
    planned.forEach((ex) => {
  ...
  const exportForLLM = async () => {
    try {
      ...
      Alert.alert(
        "Export failed",
        "Could not export workout data. Please try again.",
      );
    }
  };
        ...
        >
            ...
            {!activeIsCardio && activeIsWeight ? (
              <>
                <View
                  style={[
                    s.setHeader,
                    { borderBottomColor: t.border, backgroundColor: t.inputBg },
                  ]}
                >
                  <Text style={[s.setColHdr, { color: t.textHint }]}>
                    Body weight (kg)
                  </Text>
                </View>
              ...
              <>
                ...
                >
                  <Text style={[s.setColHdr, { flex: 1.4, color: t.textHint }]}>
                    Set
                  </Text>
                  <Text
                    style={[
                      s.setColHdr,
                      { flex: 1, textAlign: "center", color: t.textHint },
                    ]}
                  >
                    Weight kg
                  </Text>
                  ...
                  <Text
                    style={[
                      s.setColHdr,
                      { flex: 0.8, textAlign: "right", color: t.textHint },
                    ]}
                  >
                    Vol
                  </Text>
                ...
                {activeSets.map((set, i) => {
                  ...
                  return (
                    <View
                      key={i}
                      style={[s.setRow, { borderTopColor: t.border }]}
                    >
                      <Text
                        style={[
                          s.setLabel,
                      ...
                      <Text
                        style={[
                          s.setVol,
                          { flex: 0.8, color: vol > 0 ? t.accent : t.textHint },
                        ]}
                      >
                ...
                <View style={[s.totalRow, { borderTopColor: t.border }]}>
                  ...
                  <Text
                    style={[s.totalVal, { color: t.text }]}
                  >{`${Math.round(totalVol).toLocaleString()} kg`}</Text>
                </View>
              </>
            ) : null}
          </View>
          ...
          <View style={s.actionRow}>
            <TouchableOpacity
              ...
              onPress={deleteExerciseFromSession}
            >
              <Text style={[s.secondaryBtnText, { color: t.textSub }]}>
            ...
            >
              <Text style={[s.primaryBtnText, { color: t.accentText }]}>
                Save changes
              </Text>
            </TouchableOpacity>
          </View>
                    ...
                    {isWeightExercise(ex)
                      ...
                      : isCardioExercise(ex, cardioConfig)
                        ? (() => {
                            ...
                            if (entry.steps)
                              parts.push(
                                `${parseInt(entry.steps).toLocaleString()} steps`,
                              );
                            if (entry.km && cardioHasDistance(ex, cardioConfig))
                              parts.push(`${entry.km} km`);
          ...
          {exercises.length === 0 && (
            <View style={s.empty}>
              ...
              <Text style={[s.emptySub, { color: t.textSub }]}>
                This session has no logged sets to edit.
              </Text>
            </View>
          )}
  ...
  if (activeDayLogger) {
    return (
      <DayLogger
        dateKey={activeDayLogger.dateKey}
        dow={activeDayLogger.dow}
        isPastLog={true}
        onClose={() => {
          setActiveDayLogger(null);
          setPastLogDate(null);
          setPastLogDow(null);
  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: t.bg }]}>
      ...
      >
          ...
          <View style={{ flex: 1 }}>
            <View style={s.titleWrap}>
              ...
              <Text style={[s.topTitle, { color: t.text }]}>
                {selectionMode ? `${selectedCount} selected` : "History"}
              </Text>
            </View>
            <Text style={[s.topSub, { color: t.textSub }]}>
              {selectionMode
                ? "Tap sessions to select or unselect"
                : `${filteredKeys.length} session${filteredKeys.length !== 1 ? "s" : ""}`}
            </Text>
          </View>
          {selectionMode ? (
            <View style={s.selectionActions}>
              ...
              >
                <Text style={[s.smallBtnText, { color: t.text }]}>
              ...
              >
                <Text style={[s.smallBtnText, { color: t.textSub }]}>
                  Cancel
                </Text>
              </TouchableOpacity>
            </View>
        ...
        {!selectionMode && (
          <View style={s.topbarSecondaryRow}>
            <TouchableOpacity
              style={[
                s.exportBtn,
        ...
        {filteredKeys.map((k) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() =>
                selectionMode ? toggleSelect(k) : openSessionEditor(k)
              }
              activeOpacity={0.9}
            >
              <View style={s.histTopRow}>
                <View style={s.histTopLeft}>
                  ...
                  <Text style={[s.histDate, { color: t.textHint }]}>
                    {formatDate(k)}
                  </Text>
                </View>
                <View style={s.histRightRow}>
                  {vol > 0 && (
                    <Text style={[s.histVol, { color: t.accent }]}>
              ...
              <Text style={[s.histDay, { color: t.text }]}>
                {day ? day.label : "Session"}
              </Text>
              {isRest && (
                <View
                  style={[
                ...
                >
                  ...
                  <Text style={[s.restBadgeText, { color: t.textSub }]}>
                    Rest day declared
                  </Text>
                </View>
              )}
      ...
      >
        <View style={s.pastLogOverlay}>
          ...
          >
            <Text style={[s.pastLogTitle, { color: t.text }]}>
              Log a past day
            </Text>
            <Text style={[s.pastLogSub, { color: t.textSub }]}>
              Pick the date you want to log, and which day's exercises to use.
            </Text>
                ...
                <TouchableOpacity
                  ...
                  style={[
                    s.pastLogDayBtn,
                    {
                      borderColor: t.border,
                      backgroundColor:
                        pastLogDow === d.dow ? t.accent : t.inputBg,
                    },
                  ]}
                  onPress={() => setPastLogDow(d.dow)}
                >
                  <Text
                    style={[
                      s.pastLogDayBtnText,
                      { color: pastLogDow === d.dow ? "#fff" : t.text },
                    ]}
                  >
                    {d.label}
                  </Text>
            ...
            <View style={s.pastLogActions}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setActiveDayLogger({ dateKey: pastLogDate, dow: pastLogDow });
                  setPastLogModalVisible(false);
                }}
              >
                <Text style={[s.primaryBtnText, { color: t.accentText }]}>
...
const s = StyleSheet.create({
  ...
  pastLogDayBtnText: { fontSize: 12, fontWeight: "600" },
  pastLogActions: { flexDirection: "row", gap: 8, marginTop: 20 },
  topbarSecondaryRow: { flexDirection: "row", gap: 8, marginTop: 10 },
});
